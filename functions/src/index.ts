...
const upworkClientId = defineString('UPWORK_CLIENT_ID');
 ...
 */
export const fetchUpworkJobs = functions.https.onCall(
  { cors: true },
  async (request) => {
    ...
    if (!keywords || !filters) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'keywords and filters are required'
      );
    }
    ...
    try {
      ...
      if (!clientId || !clientSecret) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Upwork API credentials not configured'
        );
      }
      ...
      if (!tokenDoc.exists) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Upwork tokens not found. Please run the setup script first: node functions/setup-upwork-auth.js'
        );
      }
      ...
      const sampleJob = allJobs.find((job) => job.title?.includes('Shopify'));
      if (sampleJob) {
      ...
      uniqueJobs = uniqueJobs.filter((job) => {
        ...
        const fixedPrice = job.amount?.rawValue
      ...
      throw new functions.https.HttpsError(
        'internal',
        error.message || 'Failed to fetch jobs from Upwork'
      );
    }
  }
);
